    ...
    while let Some(Ok(hdu)) = hdu_list.next() {
        ...
        if let fitsrs::HDU::Primary(hdu) = hdu {
            ...
            if let fitsrs::Pixels::I16(it) = hdu_list.get_data(&hdu).pixels() {
                ...
                return Ok(convert_vec_to_arrayd(data, &[naxis2 as usize, naxis1 as usize])?);
            }
        }
    }
    Err(fitsrs::error::Error::DynamicError("No image data found".to_string()))
}
impl TryFrom<PathBuf> for Image {
    ...
    fn try_from(filename: PathBuf) -> Result<Self, Self::Error> {
        ...
        let data = read_fits(Fits::from_reader(BufReader::new(File::open(&filename).unwrap()))).unwrap();
        ...
        Ok(Image {
            data,
            stats,
            flat: CalibrationDescription::Flat(Flat { filter: "Foo".to_string()}),
            dark: CalibrationDescription::Dark(Dark {offset: 0, gain: 0, exposure: Duration::from_secs(0u64) }),
