...
impl<T: Debug + Sync + Send + 'static> Notify<T> {
    ...
    pub async fn subscribe(
        &self,
    ) -> tokio_stream::wrappers::BroadcastStream<Arc<T>>
    {
        let subject = self.subject.lock().await;
        let recv = self.to_notify.subscribe();
        self.to_notify.send(subject.deref().clone()).unwrap();
...
impl<'a, T> Drop for NotifyMutexGuard<'a, T> {
    ...
    fn drop(&mut self) {
        if self.should_notify {
            self.to_notify.to_notify.send(self.guard.deref().clone()).ok();
        }
        drop(&mut self.guard);
    }
}
