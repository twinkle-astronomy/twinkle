...
pub fn read_fits_from_bytes(data: &[u8]) -> Result<ArrayD<u16>, fitsrs::error::Error> {
        ...
        if let fitsrs::HDU::Primary(hdu) = hdu {
            ...
            return Ok(
                ArrayD::from_shape_vec(IxDyn(&[naxis2 as usize, naxis1 as usize]), data)
                    .map_err(|_| fitsrs::error::Error::DynamicError("Failed to create ArrayD with the given shape".to_string()))?,
            );
        }
    }
...
#[cfg(all(target_arch = "wasm32", target_feature = "simd128"))]
fn convert_bytes(bytes: &[u8], bzero: u16) -> Vec<u16> {
    let len = bytes.len() / 2;
    ...
    unsafe {
        ...
        let min_i16 = i16x8_splat(bzero as i16);
        ...
        for i in (0..simd_len).step_by(8) {
            ...
            // This is equivalent to: (x - 32768) = (x + (-32768) - 65536)
        ...
        for i in simd_len..len {
            ...
            if j + 1 < bytes.len() {
                ...
                *result_ptr.add(i) = x.wrapping_sub(bzero);
            }
        }
    }
    result
