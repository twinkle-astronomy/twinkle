...
impl<'a, 'b, K: Clone + Eq + PartialEq + Hash, V: Clone + PartialEq + Eq + Hash> ChangeAble for HashMap<K, V> {
...
mod tests {
    ...
    async fn test_hashmap() {
        let a = Arc::new(HashMap::from([
            ("a", 1)
        ]));
        let b = Arc::new(HashMap::from([
            ("a", 1),
            ("b", 2)
        ]));

        let c = Arc::new(HashMap::from([
            ("a", 4),
            ("c", 3)
        ]));
        ...
        assert_eq!(stream.next().await, Some(HashSet::from([Change::Add("a", 1)])));
        assert_eq!(stream.next().await, Some(HashSet::from([Change::Add("b", 2)])));
        assert_eq!(stream.next().await, Some(HashSet::from([Change::Delete("b"), Change::Update("a", 4), Change::Add("c", 3)])));
    }
}
