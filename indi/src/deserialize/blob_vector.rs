...
pub struct BlobIter<'a, T: std::io::BufRead> {
...
impl<'a, T: std::io::BufRead> BlobIter<'a, T> {
    pub fn new(command_iter: &mut CommandIter<T>) -> BlobIter<T> {
        BlobIter {
...
"#;
        ...
        let mut switch_iter = BlobIter::new(&mut command_iter);
