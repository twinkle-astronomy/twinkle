...
use twinkle_api::{capture::{CaptureConfig, CaptureProgress, CaptureRequest, ExposureParameterization, MessageToClient}, FromWebsocketMessage};
...
async fn task(state: Arc<AgentLock<State>>) {
    loop {
        ...
        while let Some(Ok(message)) = r.next().await {
            let msg: MessageToClient = MessageToClient::from_message(message).unwrap();
            match msg {
                MessageToClient::ExposureParameterization(exposure_parameterization) => {
                    state.write().exposure_param = Some(exposure_parameterization);
                },
                MessageToClient::Progress(status) => {
